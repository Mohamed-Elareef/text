...
def main():
    ...
    # إنشاء ThreadPoolExecutor مع 3 مسارات (threads) لتنفيذ الكود فيها
    with concurrent.futures.ThreadPoolExecutor(max_workers=process_num) as executor:
        # قم بتنفيذ الكود في كل مسار باستخدام executor.submit
        futures = [executor.submit(run_crawler, max_attempts, working_domains_file, not_working_domains_file, emails_filename, processing_domains_file) for _ in range(process_num)]

        # انتظار الانتهاء من كل المسارات
        concurrent.futures.wait(futures)

def run_crawler(max_attempts, working_domains_file, not_working_domains_file, emails_filename, processing_domains_file):
